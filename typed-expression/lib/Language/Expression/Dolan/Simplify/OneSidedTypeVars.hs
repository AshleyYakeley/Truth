...
eliminationBisubs (posvars, negvars) = let
    ...
    posbisub (MkAnyW var) =
        ...
        MkBisubstitution False var (return $ mkPolarShimWit NilDolanType) (return $ varDolanShimWit var)
    negbisub :: AnyW SymbolType -> Bisubstitution ground (DolanShim ground) Identity
    negbisub (MkAnyW var) =
        assignUVarT @TopType var $
        MkBisubstitution False var (return $ varDolanShimWit var) (return $ mkPolarShimWit NilDolanType)
