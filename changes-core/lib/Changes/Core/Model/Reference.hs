...
mapAReference MkChangeLens {..} (MkAReference refReadA refEditA objCT) =
    case transStackDict @MonadIO @tt @IO of
        Dict -> let
            ...
            refReadB mr = traceBracket "mapAReference.run.read" $ clRead refReadA mr
            refEditB :: NonEmpty (UpdateEdit updateB) -> ApplyStack tt IO (Maybe (EditSource -> ApplyStack tt IO ()))
            refEditB editbs = traceBracket "mapAReference.edit" $ do
                meditas <- traceBracket "mapAReference.edit lens" $ clPutEdits (toList editbs) refReadA
                case meditas of
                    Nothing -> do
                        traceIOM "mapAReference.edit Nothing"
                        return Nothing
                    Just [] -> return $ Just $ \_ -> return ()
                    Just (ea:editas) -> do
                        mmu <- refEditA $ ea :| editas
                        case mmu of
                            Nothing -> traceBracket "mapAReference.edit.examine: no action" $ return Nothing
                            Just mu -> traceBracket "mapAReference.edit.examine: action" $ return $ Just $ \esrc -> traceBracket "mapAReference.edit.run" $ mu esrc
