...
tupleListResourceM lt getReference =
    case lt of
        ...
        ConsListType Proxy lt' ->
            consTupleResource
                <$> (getReference FirstElementType)
                <*> (tupleListResourceM lt' $ \sel -> getReference $ RestElementType sel)

tupleResourceM ::
    forall f m sel.
    (TupleResource f, ListElementWitness sel, Applicative m) =>
...
tupleResourceM pick =
    fmap (mapResourceUpdate (tupleIsoLens fromListElementWitness toListElementWitness))
        $ tupleListResourceM representative
        $ \sel -> pick $ fromListElementWitness sel

tupleResource ::
    forall f sel.
    (TupleResource f, ListElementWitness sel) =>
...
tuplePremodel ::
    forall sel a.
    (ListElementWitness sel, Monoid a) =>
