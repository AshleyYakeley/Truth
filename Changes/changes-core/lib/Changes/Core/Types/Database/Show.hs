...
class
    (Database dbType tablesel, AllConstraint Show tablesel) =>
    ShowableDatabase (dbType :: Type) (tablesel :: Type -> Type)
    where
    ...
    showableJoinClause ::
        forall rowA rowB rowC.
        (ShowableRow dbType tablesel rowA, ShowableRow dbType tablesel rowB) =>
        JoinClause dbType tablesel rowA rowB rowC ->
        Dict (ShowableRow dbType tablesel rowC)
    showableSelectClause ::
        forall rowA rowB.
        ShowableRow dbType tablesel rowA =>
    ...
    showSelectClause ::
        forall row row'.
        ShowableRow dbType tablesel row =>
...
showableTableJoin ::
    forall dbType tablesel row.
    ShowableDatabase dbType tablesel =>
        ...
        case showableTableJoin tjoin of
            Dict ->
                "select "
                    ...
                    ++ show tjoin
                    ++ " where "
                    ++ showWhereClause @dbType @tablesel @row wc
                    ++ " order by "
                    ++ showOrderClause @dbType @tablesel @row oc
        ...
        case showableTable @dbType @tablesel row of
            Dict ->
                "update "
                    ...
                    ++ showWhereClause @dbType @tablesel @row wc
                    ++ " "
                    ++ showUpdateClause @dbType @tablesel @row uc
