...
instance
    forall cont reader.
    (Show (ContainerKey cont), AllConstraint Show reader) =>
    AllConstraint Show (KeyReader cont reader)
    where
    allConstraint = Dict

instance
    forall cont reader.
    (Show (ContainerKey cont), WitnessConstraint Show reader) =>
    WitnessConstraint Show (KeyReader cont reader)
    where
...
instance
    forall cont reader.
    (KeyContainer cont, SubjectReader reader, ReaderSubject reader ~ Item cont) =>
    SubjectReader (KeyReader cont reader)
    where
...
instance
    forall cont reader.
    (KeyContainer cont, FullSubjectReader reader, ReaderSubject reader ~ Item cont) =>
    FullSubjectReader (KeyReader cont reader)
    where
...
instance
    ...
    ApplicableEdit (KeyEdit cont edit)
    where
...
instance
    ...
    InvertibleEdit (KeyEdit cont edit)
    where
...
instance
    (FullSubjectReader (UpdateReader update), Item cont ~ UpdateSubject update) =>
    FullUpdate (KeyUpdate cont update)
    where
