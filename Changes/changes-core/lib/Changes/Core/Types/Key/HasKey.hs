...
instance
    (KeyContainer cont, HasKeyReader cont (UpdateReader updateN)) =>
    HasKeyReader cont (ContextUpdateReader updateX updateN)
    where
...
instance
    ...
    HasKeyReader [(key, val)] (PairUpdateReader keyupdate valupdate)
    where
...
instance
    ...
    HasKeyUpdate [(key, val)] (PairUpdate keyupdate valupdate)
    where
