        ...
        for_ ml $ \l -> do
            case maccel of
                ...
                Just (MkMenuAccelerator mods key) -> traceBracket "GTK.MenuBar:accel:key" $ do
                    ...
                    accelGroupConnection ag keyw gmods [AccelFlagsVisible] $ traceBracketIO "THREAD: accel" $ runWMFunction runInMain meaction
    viewBindReadOnlyWholeModel raction $ \maction ->
        liftIO $ do
            writeIORef aref maction
            set item [#sensitive := isJust maction]
    _ <- viewOn item #activate $ traceBracket "GTK.MenuBar:activate" meaction
    return ()
