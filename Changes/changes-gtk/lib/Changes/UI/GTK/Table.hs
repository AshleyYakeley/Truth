...
tableContainerView (MkKeyColumns (colfunc :: Model update -> View ( Model (WholeUpdate rowtext)
    ...
    rec
        ...
        getSelSig <- viewOn tselection #changed $ traceBracket "GTK.treeView:cursorChanged" $ runSelectNotify notifier getSelection
    let
    ...
    _ <-
        viewOn tview #rowActivated $ \tpath _ -> traceBracket "GTK.treeView:rowActivated" $ do
