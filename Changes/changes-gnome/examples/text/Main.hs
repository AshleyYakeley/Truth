                                ...
                                let
                                    ...
                                    openSelection sub =
                                        gvRunUnlocked $ do
                                            mflens <-
                                                gvLiftView
                                                    $ viewRunResource selModel
                                                    $ \selAModel -> aModelRead selAModel ReadWhole
                                            case mflens of
                                                Nothing -> return ()
                                                Just flens -> do
                                                    subSub <-
                                ...
                                in [ SubMenuEntry
                                   ...
                                   , SubMenuEntry
                                        ...
                                        [ simpleActionMenuEntry "Undo" (Just $ MkMenuAccelerator [KMCtrl] 'Z')
                                            $ liftIO
                                            $ undoHandlerUndo uh emptyResourceContext noEditSource
                                            >> return ()
                                        , simpleActionMenuEntry "Redo" (Just $ MkMenuAccelerator [KMCtrl] 'Y')
                                            $ liftIO
                                            $ undoHandlerRedo uh emptyResourceContext noEditSource
                                            >> return ()
                                        ]
                                   ]
                        (textSub, appUI) <-
                                ...
                                True -> do
                                    ...
                                    textSub2 <-
                                        gvLiftLifecycle
                                            $ makeReflectingModel
                                            $ convertReference
                                            $ mapReference textLens
                                            $ convertReference bsObj2
