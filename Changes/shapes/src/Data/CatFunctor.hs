...
endocfmap ::
       forall k (cat :: k -> k -> Type) (f :: k -> k) (a :: k) (b :: k). (CatFunctor cat cat f, InKind a, InKind b)
    => cat a b
    -> cat (f a) (f b)
endocfmap = cfmap
...
cfmap1 ::
       ...
       (CatFunctor catp (NestedMorphism catq) f, InKind a, InKind b, InKind p)
    => catp a b
    -> catq (f a p) (f b p)
cfmap1 c = unNestedMorphism $ cfmap c

cfmap2 ::
       forall catp (catq :: Type -> Type -> Type) f a b p q.
       (CatFunctor catp (NestedMorphism catq) f, InKind a, InKind b, InKind p, InKind q)
    => catp a b
    -> catq (f a p q) (f b p q)
cfmap2 c = unNestedMorphism $ unNestedMorphism $ cfmap c

cfmap3 ::
       forall catp (catq :: Type -> Type -> Type) f a b p q r.
       (CatFunctor catp (NestedMorphism catq) f, InKind a, InKind b, InKind p, InKind q, InKind r)
    => catp a b
