...
class MonadIO m =>
      IsReadableMonad m where
    type RMReader m :: * -> *
    readable :: forall t. RMReader m t -> m t

newtype Readable reader a = MkReadable
    { unReadable :: forall m. MonadIO m =>
                                  MutableRead m reader -> m a
    }
...
readableToM ::
       forall m t. IsReadableMonad m
    => Readable (RMReader m) t
    -> m t
