...
testEditor =
    let editorInit (MkObject object) = object $ \muted -> mutableRead muted ReadWhole
...
testSavable =
    testCase "Savable" $ do
        ...
        let saveSub = saveBufferSubscriber sub
...
testOutputEditor ::
    ...
    -> Editor edit actions ()
testOutputEditor name call =
    let outputLn :: MonadIO m => String -> m ()
        ...
        editorDo obj subActions =
            let subDontEdits :: [[edit]] -> IO ()
...
testSubscription name initial call =
    goldenTest' name $ do
        var <- newMVar initial
        let varObj :: Object (WholeEdit (EditSubject edit))
            varObj = mvarObject var $ \_ -> True
            editObj :: Object edit
            editObj = convertObject varObj
        sub <- makeObjectSubscriber editObj
        let ?showVar = withMVar var $ \s -> hPutStrLn ?handle $ "var: " ++ show s
            ?showExpected = \edits ->
                withMVar var $ \s -> do
