module Truth.Core.Read.ReadFunction where
{
    ...
    instance MapReadable (WriterReadable w) where
    {
        ...
        mapReadableF (rff :: ReadFunctionF f ra rb) (MkWriterReadable sbwt) = MkWriterReadable $ \(sa :: MutableRead m ra) wm -> getCompose $ sbwt (mapStructureF rff sa) (fmap (Compose . fmap pure) wm);
    };

    writerToReadable :: forall w reader. WriterReadable w reader () -> Readable reader [w];
    writerToReadable (MkWriterReadable swma) = MkReadable $ \(s :: forall t. reader t -> m t) -> execWriterT $ swma (fmap lift s) (tell . pure);
}
