...
class
    (FiniteWitness colsel, WitnessConstraint Show colsel, AllConstraint Show colsel) =>
...
sqlitePinaforeLens = let
        ...
        row <-
            mr
                $ DatabaseSelect
                    ...
                    ( MkTupleWhereClause
                        $ (ColumnExpr TriplePredicate === ConstExpr p)
                        /\ (ColumnExpr TripleSubject === ConstExpr s)
                    )
                    mempty
        ...
        row <-
            mr
                $ DatabaseSelect
                    ...
                    ( MkTupleWhereClause
                        $ (ColumnExpr TriplePredicate === ConstExpr p)
                        /\ (ColumnExpr TripleValue === ConstExpr v)
                    )
                    mempty
    ...
    clPutEdit (QTableEditPropertySet p s Nothing) =
        return
            ...
            $ MkTupleWhereClause
            $ ColumnExpr TriplePredicate
            === ConstExpr p
            /\ ColumnExpr TripleSubject
            === ConstExpr s
    ...
    clPutEdit (QTableEditEntityRefCount v Nothing) =
        return
            $ Just
            $ pure
            $ DatabaseDelete (MkTupleTableSel QSModelCount)
            $ MkTupleWhereClause
            $ ColumnExpr RefCountKey
            === ConstExpr v
    ...
    clPutEdit (QTableEditLiteralSet v Nothing) =
        return
            $ Just
            $ pure
            $ DatabaseDelete (MkTupleTableSel QSLiteral)
            $ MkTupleWhereClause
            $ ColumnExpr LiteralKey
            === ConstExpr v
    clPutEdits ::
        forall m.
        MonadIO m =>
