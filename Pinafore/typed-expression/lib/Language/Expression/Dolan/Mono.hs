...
class
    (IsDolanGroundType ground, IsCovaryGroundType conc) =>
    CovarySubtype (ground :: GroundTypeKind) (conc :: forall k. k -> Type)
    where
    dolanToMonoGroundType :: forall dv t. ground dv t -> Maybe (CovaryType dv, conc t)

nonpolarGroundedToMonoType ::
    forall (ground :: GroundTypeKind) (conc :: forall k. k -> Type) a.
