...
instance
    ...
    (forall sv a. HasRecursion (w sv a)) =>
    HasRecursion (CCRArguments w dv gt t)
    where
...
instance
    forall (ground :: GroundTypeKind) ft polarity sv t.
    ( Is PolarityType polarity
    , forall polarity' t'. Is PolarityType polarity' => ExtractEquations ground (ft polarity' t')
    ) =>
    ExtractEquations ground (CCRPolarArgument ft polarity sv t)
    where
...
instance
    forall (ground :: GroundTypeKind) (w :: CCRArgumentKind) dv gt t.
    ( forall sv a.
      ExtractEquations ground (w sv a)
    ) =>
    ExtractEquations ground (CCRArguments w dv gt t)
    where
...
instance
    forall (ground :: GroundTypeKind) polarity t.
    Is PolarityType polarity =>
    ExtractEquations ground (DolanGroundedType ground polarity t)
    where
...
instance
    forall (ground :: GroundTypeKind) polarity t.
    Is PolarityType polarity =>
    ExtractEquations ground (DolanSingularType ground polarity t)
    where
...
instance
    forall (ground :: GroundTypeKind) polarity t.
    Is PolarityType polarity =>
    ExtractEquations ground (DolanType ground polarity t)
    where
        ...
        Nothing ->
            case mergeGroundMaybe ts tr of
                Just (MkShimWit tsr conv) ->
                    Just
                        $ MkShimWit (ConsDolanType t1 tsr)
                        $ polarF (polar2 . conv . polar1) (iPolarPair id $ conv . polar2)
                Nothing -> Nothing
