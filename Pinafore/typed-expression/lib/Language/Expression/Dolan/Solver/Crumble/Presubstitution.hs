...
data SubShimWit (ground :: GroundTypeKind) (polarity :: Polarity) oldtv newtv =
    forall t. MkSubShimWit (DolanType ground polarity t)
                           (DolanPolarShim ground polarity oldtv (JoinMeetType polarity newtv t))

instance forall (ground :: GroundTypeKind) polarity oldtv newtv. (ShowGroundType ground, Is PolarityType polarity) =>
             Show (SubShimWit ground polarity oldtv newtv) where
...
cleanSubShimWit ::
       forall (ground :: GroundTypeKind) polarity oldtv newtv. (IsDolanGroundType ground, Is PolarityType polarity)
    => TypeVarT oldtv
...
assignPresubstitution oldvar mta mtb = do
    ...
    return $
        assignTypeVarT @(MeetType (JoinType newtv a) b) oldvar $ let
            toSubShimWitPos ::
                   forall .
...
instance forall (ground :: GroundTypeKind). ShowGroundType ground => Show (Presubstitution ground) where
    show (MkPresubstitution oldvar newvar ta tb) = let
        ...
        in "{" <>
           show oldvar <>
           "+ => " <>
           show newvar <> " | " <> showM ta <> "; " <> show oldvar <> "- => " <> show newvar <> " & " <> showM tb <> "}"
