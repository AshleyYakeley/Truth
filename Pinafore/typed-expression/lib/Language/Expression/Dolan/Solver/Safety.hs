...
instance
    forall (ground :: GroundTypeKind) sv polarity t.
    IsDolanGroundType ground =>
    CheckSafety (CCRPolarArgument (DolanType ground) polarity sv t)
    where
...
instance
    forall (ground :: GroundTypeKind) dv gt polarity t.
    IsDolanGroundType ground =>
    CheckSafety (CCRPolarArguments dv (DolanType ground) gt polarity t)
    where
...
instance
    ...
    IsDolanGroundType ground =>
    CheckSafety (DolanSingularType ground polarity t)
    where
...
instance
    forall (ground :: GroundTypeKind) polarity t.
    IsDolanGroundType ground =>
    CheckSafety (DolanType ground polarity t)
    where
...
instance
    forall (ground :: GroundTypeKind) polarity t.
    IsDolanGroundType ground =>
    CheckSafety (FlipType ground polarity t)
    where
