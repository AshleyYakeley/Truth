...
data ErrorType
    ...
    | LookupAmbiguousError FullNameRef
                           [FullName]
...
instance Show ErrorType where
    ...
    show (LookupNamesUnknownError nn) = "undefined or ambiguous names: " <> (intercalate ", " $ fmap show $ toList nn)
    show (LookupNotDefinedError n) = "undefined: " <> show n
    show (LookupAmbiguousError n nn) = "ambiguous: " <> show n <> ", could be " <> (intercalate ", " $ fmap show nn)
