        ...
        let
            ...
            getColumn (nameRef, getCellRef) = let
                ...
                getCellSub osub = do
                    ...
                    return
                        $ unWModel
                        $ eaMapSemiReadOnly (funcChangeLens showCell)
                        $ langWholeModelToReadOnlyValue
                        $ getCellRef a
...
uiList mkWidget listModel =
    MkLangWidget $ \ec ->
        createListBox (\model -> unLangWidget (mkWidget $ functionImmutableModel $ MkWModel model) ec)
            $ unWModel
            $ langListModelToOrdered listModel
...
uiUnitCheckBox name val =
    MkLangWidget $ \_ ->
        createCheckButton (unWModel $ immutableWholeModelValue mempty name)
            $ unWModel
            $ eaMap (toChangeLens knowBool) val
...
langImage ref =
    MkLangWidget $ \_ ->
        createImage
            $ unWModel
            $ eaMapReadOnlyWhole (fmap (someConvertImage . unLangImage) . knowToMaybe)
            $ immutableModelToReadOnlyModel ref
...
uiAttachScaleMarks model sw =
    MkLangScaleWidget $ \c -> do
        ...
        attachScaleMarks scale
            $ unWModel
            $ eaMapReadOnlyWhole (fmap $ \(v, (p, mt)) -> (v, p, mt))
            $ immutableWholeModelValue mempty model
        return scale
    ...
    headingBDS
        ...
        , namespaceBDS
            ...
            , hasSubtypeRelationBDS @LangWidget @LangLayoutWidget Verify ""
                $ functionToShim "layout widget"
                $ MkLangLayoutWidget defaultLayoutOptions
