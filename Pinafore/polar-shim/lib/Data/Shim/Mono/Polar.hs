...
instance
    forall polarity k (shim :: ShimKind k) (a :: k) (b :: k).
    Show (PolarShimType shim polarity a b) =>
    Show (PolarShim shim polarity a b)
    where
    show (MkPolarShim shim) = show shim

instance
    forall polarity k (shim :: ShimKind k).
    (Is PolarityType polarity, Category shim) =>
    Category (PolarShim shim polarity)
    where
    id =
...
instance
    forall polarity k (shim :: ShimKind k).
    (Is PolarityType polarity, Groupoid shim) =>
    Groupoid (PolarShim shim polarity)
    where
    invert =
        case polarityType @polarity of
...
instance
    forall polarity (shim :: ShimKind Type).
    (Is PolarityType polarity, CartesianShim shim) =>
    CartesianShim (PolarShim shim polarity)
    where
    funcShim =
        case polarityType @polarity of
...
reshimPolarShim ::
    forall polarity k (shim1 :: ShimKind k) (shim2 :: ShimKind k) (a :: k) (b :: k).
    Is PolarityType polarity =>
