...
predefinedBindings =
    mconcat
        ...
        -- drag
        -- icon
        , qbind "uiPick" $ \(nameMorphism :: PinaforeFunctionMorphism Point (Maybe Text)) (fset :: PinaforeFunctionValue (FiniteSet Point)) ->
              let getName :: PinaforeFunctionMorphism Point (Maybe Point, String)
                  getName = proc p -> do
                    n <- nameMorphism -< p
                  ...
                  opts ::
                         GeneralFunction PinaforeEdit (ListEdit [(Maybe Point, String)] (WholeEdit (Maybe Point, String)))
                  opts =
        ...
        -- switch
        , qbind "uiTable" $ \cols (asp :: Point -> Result String (Text, UISpec PinaforeEdit)) (val :: PinaforeLensValue (FiniteSetEdit Point)) ->
              let showCell :: Maybe String -> (String, TableCellProps)
                  showCell (Just s) = (s, tableCellPlain)
                  showCell Nothing = ("empty", tableCellPlain {tcItalic = True})
                  mapLens ::
                         PinaforeLensValue (WholeEdit (Maybe Point)) -> PinaforeFunctionValue (String, TableCellProps)
                  mapLens lens =
