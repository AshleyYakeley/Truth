...
module Main(main) where
{
    ...
    [
        ...
        testQueryValue "let a=1;b=2 in (a,b,a,b)" (qlet "a" (pure $ qint 1) $ qlet "b" (pure $ qint 2) $ (,,,) <$> (qvar "a") <*> (qvar "b") <*> (qvar "a") <*> (qvar "b")) $ Just (qint 1,qint 2,qint 1,qint 2),
        testQueryValue "let a=1;b=2 in (b,a,b,a)" (qlet "a" (pure $ qint 1) $ qlet "b" (pure $ qint 2) $ (,,,) <$> (qvar "b") <*> (qvar "a") <*> (qvar "b") <*> (qvar "a")) $ Just (qint 2,qint 1,qint 2,qint 1)
    ];
    ...
    tests :: TestTree;
    tests = localOption (mkTimeout 2000000) $ testGroup "pinafore"
    [
        testQueryValues,
        testQueries
    ];
