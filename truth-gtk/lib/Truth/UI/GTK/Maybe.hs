module Truth.UI.GTK.Maybe (oneGetView) where
{
    ...
    (
      ...
      (forall editb. (FullEdit editb) => UISpec editb -> UISpec (OneWholeEdit f editb)) -> Maybe (Limit f) -> (Object (OneWholeEdit f edit) -> IO wd) -> GCreateView edit -> GCreateView (OneWholeEdit f edit);
    oneWholeView uispec mDeleteValue makeEmptywidget baseView = do
    {
        ...
        mDeleteButton <- liftOuter $ liftIOView $ \unlift -> for mDeleteValue $ \(MkLimit deleteValue) -> makeButton "Delete" $ unlift $ viewMutableEdit $ \muted -> pushMutableEdit muted [SumEditLeft $ MkWholeEdit deleteValue];
        ...
        let
        {
            update :: forall m. IsStateIO m => MutableRead m (OneReader f (EditReader edit)) -> [OneWholeEdit f edit] -> m ();
            update mr wedits = mvarStateAccess stateVar $ do
            {
                oldfvr <- get;
                newfu <- lift $ mr ReadHasOne;
                case (retrieveOne oldfvr,retrieveOne newfu) of
                {
                    (SuccessResult vr,SuccessResult ()) -> lift $ vrUpdate (mapViewResultEdit (mustExistOneGeneralLens "object") vr) mr wedits;
