...
swapStateT (StateT s1s2a) =
    StateT $ \olds2 ->
        StateT $ \olds1 -> fmap (\((a, news1), news2) -> ((a, news2), news1)) $ runStateT (s1s2a olds1) olds2
...
lensStateT MkLens {..} (StateT pma) =
    StateT $ \oldw -> fmap (\(a, newp) -> (a, runIdentity $ lensPutback newp oldw)) $ pma (lensGet oldw)
...
pairStateAccess rsam rsbm sabr =
    rsam $ StateT $ \oldsa -> rsbm $ StateT $ \oldsb -> fmap swap3 $ runStateT sabr (oldsa, oldsb)
